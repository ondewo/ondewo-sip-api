...
githubio_wrapper:
	...
	@! cat ondewo.github.io/index.html | grep "${REPO_NAME} API" | grep -q ${ONDEWO_SIP_API_VERSION} || (echo "Already updated ${REPO_NAME} to ${ONDEWO_SIP_API_VERSION}" & rm -rf ondewo.github.io && exit 1)
	@sed -i "s/${REPO_NAME} API.*\</${REPO_NAME} API ${ONDEWO_SIP_API_VERSION}\<\//" ondewo.github.io/index.html
	@cat ondewo.github.io/index.html | grep "${REPO_NAME} API"
	@git -C ondewo.github.io add index.html
	@git -C ondewo.github.io commit -m "Updated ${REPO_NAME} to ${ONDEWO_SIP_API_VERSION}"
