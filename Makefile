...
ONDEWO_SIP_API_VERSION=5.1.0

# You need to setup an access token at https://github.com/settings/tokens - permissions are important
GITHUB_GH_TOKEN?=ENTER_YOUR_TOKEN_HERE

CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| sed -n '/Release ONDEWO SIP API ${ONDEWO_SIP_API_VERSION}/,/\*\*/p'`
...
githubio_logic_pre:
	...
	@sed -i "/{ number: '${ONDEWO_SIP_API_VERSION}', link: 'ondewo-${REPO_NAME}-api\/${ONDEWO_SIP_API_VERSION}\/' },/d" ondewo.github.io/data.js
	@rm -rf ${DOCS_DIR}
	@mkdir "${DOCS_DIR}"
	@cp docs/* ${DOCS_DIR}
...
githubio_logic: | githubio_logic_pre
	...
	@! cat ondewo.github.io/data.js | sed -n "/name\: '${REPO_NAME_UPPER}'/,/end\: ''/p" | grep -q "number: '${ONDEWO_SIP_API_VERSION}'" || (echo "Already Released" && exit 1)
	...
	$(eval TEMP_TEXT:= $(shell cat ondewo.github.io/script_object.txt | sed -e "s/VERSION/${ONDEWO_SIP_API_VERSION}/g" -e "s/TECHNOLOGY/${REPO_NAME}/g"))
...
# Change Version Number and RELEASE NOTES
	...
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_SIP_VERSION.*=.*/ONDEWO_SIP_VERSION = ${ONDEWO_SIP_API_VERSION}/' Makefile
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_PROTO_COMPILER_GIT_BRANCH.*=.*/ONDEWO_PROTO_COMPILER_GIT_BRANCH=tags\/${PROTO_COMPILER}/' Makefile
	cd ${REPO_DIR} && sed -i -e 's/SIP_API_GIT_BRANCH.*=.*/SIP_API_GIT_BRANCH=tags\/${ONDEWO_SIP_API_VERSION}/' Makefile && head -30 Makefile
