...
IMAGE_UTILS_NAME=ondewo-sip-api-utils:${ONDEWO_SIP_API_VERSION}
.DEFAULT_GOAL := help
...
install_python_requirements: ## Installs python requirements flak8 and mypy
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-sip-client-python/master/requirements.txt -O requirements.txt
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-sip-client-python/master/requirements-dev.txt -O requirements-dev.txt
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-sip-client-python/master/.flake8 -O .flake8
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-sip-client-python/master/mypy.ini -O mypy.ini
...
githubio_logic_pre:
	...
	$(eval DOCS_DIR:=ondewo.github.io/docs/ondewo-${REPO_NAME}-api/${ONDEWO_SIP_API_VERSION})
	@perl -i -ne "print unless /{ number: '${ONDEWO_SIP_API_VERSION}', link: 'ondewo-${REPO_NAME}-api\/${ONDEWO_SIP_API_VERSION}\/' },/" ondewo.github.io/data.js
	@rm -rf ${DOCS_DIR}
	@mkdir "${DOCS_DIR}"
	@cp docs/* ${DOCS_DIR}
	@perl -i -pe "s/h1>Protocol Documentation/h1>${REPO_NAME_UPPER} ${ONDEWO_SIP_API_VERSION} Documentation/" ${DOCS_DIR}/index.html

githubio_logic: | githubio_logic_pre
	$(eval REPO_NAME:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 ))
	$(eval REPO_NAME_UPPER:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 | perl -pe 's/(.*)/\U$$1/'))
	@git branch | grep "*" | grep -q "master" || (echo "Not on master branch"  & rm -rf ondewo.github.io && exit 1)
	@! cat ondewo.github.io/data.js | perl -ne "print if /name\: '${REPO_NAME_UPPER}'/../end\: ''/" | grep -q "number: '${ONDEWO_SIP_API_VERSION}'" || (echo "Already Released" && exit 1)
	...
	$(eval TEMP_TEXT:= $(shell cat ondewo.github.io/script_object.txt | perl -pe "s/VERSION/${ONDEWO_SIP_API_VERSION}/g; s/TECHNOLOGY/${REPO_NAME}/g"))
	...
	@cp docs/* ${DOCS_DIR}
	@perl -i -pe "s/h1>Protocol Documentation/h1>${REPO_NAME_UPPER} ${ONDEWO_SIP_API_VERSION} Documentation/" ${DOCS_DIR}/index.html
	$(eval HEADER_LINE:= $(shell cat ${DOCS_DIR}/index.html | grep -n "${REPO_NAME_UPPER} ${ONDEWO_SIP_API_VERSION} Documentation" | grep -o -E '[0-9]+' | head -1 | perl -pe 's/^0+//'))
	...
	@git -C ondewo.github.io status
	@git -C ondewo.github.io commit -m "Added ${REPO_NAME} Documentation for ${ONDEWO_SIP_API_VERSION}"
...
create_release_branch: ## Create Release Branch and push it to origin
	git checkout -b "release/${ONDEWO_SIP_API_VERSION}"
	git push -u origin "release/${ONDEWO_SIP_API_VERSION}"

create_release_tag: ## Create Release Tag and push it to origin
	git tag -a ${ONDEWO_SIP_API_VERSION} -m "release/${ONDEWO_SIP_API_VERSION}"
...
release_all_clients:
	@rm -f .already_released_marker; make release_python_client || { [ -f .already_released_marker ] && echo "SKIP: Python client already released ${ONDEWO_SIP_API_VERSION}" || { echo "FAILED: Python client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_nodejs_client || { [ -f .already_released_marker ] && echo "SKIP: Nodejs client already released ${ONDEWO_SIP_API_VERSION}" || { echo "FAILED: Nodejs client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_typescript_client || { [ -f .already_released_marker ] && echo "SKIP: Typescript client already released ${ONDEWO_SIP_API_VERSION}" || { echo "FAILED: Typescript client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_angular_client || { [ -f .already_released_marker ] && echo "SKIP: Angular client already released ${ONDEWO_SIP_API_VERSION}" || { echo "FAILED: Angular client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_js_client || { [ -f .already_released_marker ] && echo "SKIP: JS client already released ${ONDEWO_SIP_API_VERSION}" || { echo "FAILED: JS client did not release - aborting"; exit 1; }; }
...
release_client:
	...
	$(eval REPO_DIR:= $(shell echo "ondewo-sip-client-${REPO_NAME}"))
...
NODEJS_CLIENT="git@github.com:ondewo/ondewo-sip-client-nodejs.git"
...
TYPESCRIPT_CLIENT="git@github.com:ondewo/ondewo-sip-client-typescript.git"
...
ANGULAR_CLIENT="git@github.com:ondewo/ondewo-sip-client-angular.git"
...
JS_CLIENT="git@github.com:ondewo/ondewo-sip-client-js.git"
